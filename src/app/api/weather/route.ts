...
const weatherLogger = logger.withDefaults({ tag: 'api:weather' });
...
export const POST = async (req: Request) => {
  try {
    ...
    if (data.error) {
      weatherLogger.error('Failed to fetch weather data.', {
        reason: data.reason,
      });
      return Response.json(
        {
    ...
    weatherLogger.error('Failed to resolve weather widget data.', err);
    return Response.json(
      {
        message: 'An error has occurred.',
      },
